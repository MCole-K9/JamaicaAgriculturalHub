...
<div class="row">
    ...
    <div class="row">
        <asp:Label runat="server" ID="lblEmail" CssClass="text col-lg-3 input-sm">Email:</asp:Label>
        ...
        <asp:RegularExpressionValidator runat="server" ID="regEmail" ControlToValidate="txtEmail" ErrorMessage="Field Must be a valid email format, for example: &quot;example@email.com&quot;" 
            ValidationExpression="\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*" CssClass="text text-danger col-lg-3 input-sm" />
    </div>
    <div class="row">
